...
def run_realization(realization_idx, num_cells_target, steps, L_str):
    ...
    using UNWEIGHTED updates (simple average of reflections).
    """
    ...
    # --- Precalculate Reflection Info (Unweighted) ---
    for i in range(N):
        reflection_info = []
        if cells[i] is None:
            continue
        ...
        for u, v in zip(c_verts, c_verts[1:] + c_verts[:1]):
            ...
            if len(cs) == 2:
                ...
                reflection_info.append((core_j, shift_j, u, v))
    ...
    for step in range(steps):
        ...
        for i in range(N):
            ...
            if refinfo:
                # Apply PBC shifts to reflections
                for core_j, shift_j, u, v in refinfo:
...
if __name__ == '__main__':
    ...
    print(f"Starting UNWEIGHTED parallel ensemble: {num_realizations} realizations, {num_cells_target} cells each.")
    ...
    if final_plot_data is not None:
        ...
        plt.axis('equal')
        plt.title(f"Sample Geometry (Realization 0, {Nc} Unweighted Periodic Cells)")
    ...
    plt.title(f"Unweighted Ensemble Convergence Limit ({num_realizations} Realizations, {num_cells_target} Cells)")
